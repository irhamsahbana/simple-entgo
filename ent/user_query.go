...
func (uq *UserQuery) loadTodo(ctx context.Context, query *TodoQuery, nodes []*User, init func(*User), assign func(*User, *Todo)) error {
	...
	query.Where(predicate.Todo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TodoColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_todo
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_todo" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_todo" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
